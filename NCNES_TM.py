...
""")
    ...
    def fit(self):
        for ep in range(self.eposides):
            ...
            for i in range(self.distribution_size):
                ...
                mean += self.learning_rate_m * (np.linalg.inv(fisher_m) @ f_m_grad)  # + self.phi * d_m_grad)
                cov += self.learning_rate_cov * (np.linalg.inv(fisher_cov) @ f_cov_grad)  # + self.phi * d_cov_grad)
    ...
    def f_cov_grad(self, X, mean, cov_inv, rewards):
        ...
        X_dif = X - mean
        for i in range(self.pop_size):
            grad += (cov_inv @ np.outer(X_dif[i], X_dif[i]) @ cov_inv - cov_inv) * rewards[i]
    ...
    def fisher_m(self, X, mean, cov_inv):
        ...
        X_dif = X - mean
        for i in range(self.pop_size):
            grad += cov_inv @ np.outer(X_dif[i], X_dif[i]) @ cov_inv
        grad /= self.pop_size
        return grad
    ...
    def fisher_cov(self, X, mean, cov_inv):
        ...
        X_dif = X - mean
        for i in range(self.pop_size):
            y = cov_inv @ np.outer(X_dif[i], X_dif[i]) @ cov_inv - cov_inv
            grad += y @ y.T
